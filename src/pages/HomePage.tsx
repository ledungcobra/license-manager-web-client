...
const HomePage = ({}: Props) => {
    ...
    React.useEffect(() => {
        if (rootContract === null ||
            rootContract === undefined ||
            Object.keys(rootContract!!).length === 0 ||
            !currentAccount) {
            return;
        }

        rootContract.getApps().then(apps =>
            setRootState((prev) => ({...prev, apps: [...apps]})))
            .catch((e) => {
                console.log(e)
                setAlertType({
                    type: 'error',
    ...
    const ownerElements = [
        <div className='flex justify-between'>
            <strong>Balance: </strong><span>{rootState.balance}</span>
        </div>,
        <div className='flex justify-between'>
            <strong>Application contract counts: </strong><span>{rootState.apps.length}</span>
        </div>,
        <div className='flex justify-between'>
            <strong>Registration app fee: </strong><span>{rootContract.licenseRegistrationFee}</span>
        </div>
    ]
    ...
    const ownerActions = [
        <Button danger className='bg-red-200' onClick={() => widthDraw()}>Withdraw</Button>,
    ]

    const normalActions = [
        <Button type='primary' ghost onClick={() => setShowCreateAppDialog(true)}>Create New App</Button>
    ]
    ...
    const onSubmitCreateAppLicense = async (values: CreateAppLicenseFormField) => {
        ...
        try {
            await rootContract.createNewLicenseToken(values.appName,
                values.appImageUrl,
                values.description,
                values.price,
                values.duration, values.unit as Unit,values.secret);
            setAlertType({type: 'success', message: 'Create new app success', title: ''});
        } catch (e: any) {
    ...
    const createAppDialog = <>
        <Card size="default" title={"Create new app"}
              extra={
        ...
        >
            ...
            >
                ...
                >
                    <Input
                        placeholder={'Functionality of your app and your contact number or your company'}/>
                </Form.Item>
                <Form.Item
                    label="Price"
                    name="price"
                ...
                {
                    unitTypeSet !== Unit.FOREVER + '' && <Form.Item
                        label="Duration"
                        name="duration"
                        rules={[{
                            required,
                    ...
                    >
                        <InputNumber placeholder='Duration on token will last'
                ...
                >
                    <Select style={{width: 120}}
                        ...
                        <Select.Option value="0" key='0'>Second</Select.Option>
                        <Select.Option value="1" key='1'>Day</Select.Option>
                        <Select.Option value="2" key='2'>Month</Select.Option>
                        <Select.Option value="3" key='3'>Year</Select.Option>
                        <Select.Option value="4" key='4'>Forever</Select.Option>
                    </Select>
                </Form.Item>
                <Form.Item label='App Secret' name='secret'
        ...
        <>
            <div className="flex flex-wrap gap-2 font-bold w-full mx-auto flex justify-center flex-col items-center">
                <div className='w-1/2'>
                    ...
                    <Card size="default" title={"Contract address: " + rootState.address}
                          extra={<a href="#">More</a>}
                    >
                        {
                            rootState.isOwner &&
