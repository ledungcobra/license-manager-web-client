...
export const Line = () => {
    return <hr className="w-full h-1 mb-2"/>;
};
...
const AppItem = ({
    ...
    useEffect(() => {
        ...
        app.listen()
            ...
            .on(AppContractEvents.BalanceChange, ({contractAddress, amount}) => {
                if (contractAddress.toLowerCase() === app.address.toLowerCase()) {
                    app.balance = amount;
                    updateItem?.(index, app);
                }
            });
    ...
    const byeLicenseModal = useMemo(() => {
        return (
            <Card size="small" title="Buy license">
                <Form
                    initialValues={
                        {
                            ...
                            nTokens: 5,
                        } as PurchaseLicenseForm
                    }
                    onFinish={handleBuyLicense}
                >
                    <Form.Item
                        label="Address"
                        name="address"
    ...
    return (
        <div className={`${app.owner === currentAccount ? "w-1/3" : "w-1/4"} border-2`}>
            <div className={className}>
                ...
                <Line/>
                <div className="flex justify-between w-full overflow-ellipsis flex-wrap">
                    <strong>Owner address: </strong>
                    <span className="text-blue-600 hover:text-blue-800 cursor-pointer">{app.owner}</span>
                </div>
                <Line/>
                <div className="flex justify-between w-full overflow-ellipsis flex-wrap">
                    <strong>App address: </strong>
                    <span className="text-blue-600 hover:text-blue-800 cursor-pointer">{app.address}</span>
                </div>
                <Line/>
                ...
                <Line/>
                <div className="flex justify-between w-full">
                    <strong>Token License Price: </strong>
                    {app.price}
                </div>
                <Line/>
                <div className="flex justify-between w-full">
                ...
                <Line/>
                <div className="flex justify-between w-full">
                    <strong>Duration per token: </strong>
                    {app.durationPerToken}
                </div>
                <Line/>
                <div className="flex justify-between w-full">
                    <strong>Duration unit: </strong>
                    {Unit[app.unit]?.toLowerCase()}
                </div>
                <Line/>
                ...
                <div className="flex justify-between w-full">
                    {!viewOnly ?
                        (
                            ...
                            (
                                !app.active ?
                                    <Button onClick={activateApp}>Active</Button>
                                    :
                                    <Button onClick={deActivateApp}>DeActive</Button>
                        ...
                        (
                            !app.active && (
                                <span className="text-red-400 font-bold">App is not active yet so you cannot buy the license</span>
                            )
                        )}
                    {app.active && (
                        <>
